...
function SignUpPage() {
  ...
  return (
    <Container>
      ...
      <Input label="닉네임" type="text" placeholder="닉네임을 입력해주세요" margin="auto" />
      {/* Email */}
      <EmailInput
        label="이메일"
        type="text"
      ...
      {/* Password */}
      <Input marginLabel="18px 255px 0 0" label="비밀번호" type="password" placeholder="비밀번호를 입력해주세요" margin="auto" value={password} onChange={onPasswordChange} />
      ...
      */}
      <CardSelect label="카드 등록" text="카드를 선택해주세요" options={cards.map((obj) => obj.name)} onChange={onCardChange} margin="0" />
      <FlexContainer>
        {userCardList.map((obj) => (
          <CardList key={obj.id} text={obj.name} onClick={() => onCardDelete(obj.id)} />
        ))}
      </FlexContainer>
      {/* Button */}
      <Link to="/login">
